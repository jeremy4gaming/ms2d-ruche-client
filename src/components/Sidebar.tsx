...
export const Sidebar: React.FC<SidebarProps> = ({ collapse, onToggle }) => {
  ...
  return (
    <aside className={`bg-white h-screen shadow-md transition-all duration-300 relative ${collapse ? 'w-20' : 'w-64'}`}>
      <div className="h-full flex flex-col justify-between">
        <div>
          {/* Logo et titre */}
          <div className="flex items-center h-16 border-b px-4">
            {!collapse && (
              <h1 className="text-xl font-bold text-blue-600">Equipe E - Ruche</h1>
            )}
            {collapse && (
        ...
        {/* Le bouton de déconnexion a été retiré */}
      </div>
      
      {/* Bouton de réduction/expansion de la sidebar */}
      <button
        onClick={onToggle}
        className="absolute -right-4 top-20 bg-white rounded-full p-1 shadow-md border border-gray-200 hover:bg-gray-100 transition-colors"
